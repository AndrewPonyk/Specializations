...
def predict(rnn, word, n_predictions=n_categories):
    ...
    # Turn word into tensor that we can use for language prediction.
    word_tensor=wtotensor(word)

    # Initialize our hidden variable.
    hidden = rnn.initHidden()

    # Use network to detect language for each character.
    for i in range(word_tensor.size()[0]):
        output, hidden = rnn(word_tensor[i], hidden)

    # Get the output in numpy array and figure out the languages that we've detected.
