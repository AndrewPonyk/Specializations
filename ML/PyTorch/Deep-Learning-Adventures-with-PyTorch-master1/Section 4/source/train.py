...
def sentence_to_idx(lang, sentence):
    """ Encode sentences to indexes in our Vocabulary object for a given langauge. """
    out=[]
...
def sentence_to_tensor(lang, sentence):
    """ Turn a sentence into a tensor.     Add EOS_token at the end of the new tensor to mark end of the sentence. """
...
def train(input_tensor, output_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, loss_func, max_length=MAX_SENTENCE_LENGTH):
    ...
    for di in range(output_length):
        ...
        if decoder_input.item() == EOS_token:
            break
    # Clean up the "gradients" before propagating changes to our network.
...
def train_all(pairs, encoder, decoder, il, ol, s_epochs, print_every=10):
    """ Train on a s_epochs random pair of sentences using encoder and decoder. print_every - show stats on print_every sentence """
    loss_total = 0
...
def test(encoder, decoder, sentence, input_lang, output_lang, max_length=MAX_SENTENCE_LENGTH):
    ...
    # This is not training, so we can save up some memory.
    with torch.no_grad():
        # Prepare sentence for translation.
        input_tensor = sentence_to_tensor(input_lang, sentence)
        input_length = input_tensor.size()[0]

        # This is similar to training, but without running .zero_grad(), .backward(), .set()
        encoder_hidden = encoder.initHidden()
        ...
        # Prepare data for the second network based on the output of the first one.
        decoder_input = torch.tensor([[SOS_token]], device=device)

        decoder_hidden = encoder_hidden

        # Get the translation using the second network decode it's output.
        decoded_words = []
        for di in range(max_length):
            ...
            if topi.item() == EOS_token:
                ...
                # otherwise just turn the encoded translation (as indexes) back to their respective words.
...
def test_random(encoder, decoder,ilang,olang, n=5):
    """ Randomly get a pair of sentences and compare them with our translation. """
    for i in range(n):
...
if __name__ == '__main__':
    ...
    # Get maximum of 100 sentences. Remember that in prep.py we get only questions that match secific criteria.
