...
Before you can use this script you need to install a unidecode package with: pip3 install unidecode """
...
class Vocab:
    ...
    def add_word(self, word):
        """        Add a new word to a vocabulary, update all the counters and indexes."""
...
def isquestion(s, max_length=MAX_SENTENCE_LENGTH):
    """    Return True if sentence is valid according to our criteria.
    Here we're interested in questions that are no longer than mex_legth. """
    return len(s.split(' ')) < max_length and len(s.split(' ')) < max_length and s.find('?') != -1

def clean(s, extra_cleaner=qscleaner):
    """    Clean up the whole sentence: Include only words, make
    them lower case and remove any non-english characters. """
    include_words=[]
    for word in s.split():
...
def process_file(ilang, olang, limit, sfilter=isquestion):
    """ Read a language file, clean up sentences and based on them create a Vocab object for each language, return only limit sentences. """
...
def get_data(ilang, olang, limit=100, log=print):
    """ Return a limit number of sentences of both ilang and olang. Sentences has to match criteria defined by sfilter and are processed by wclean.
